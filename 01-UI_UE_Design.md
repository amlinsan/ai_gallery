...
- **背景替换入口**: 左上角新增“更换背景”按钮（`btn_bg_replace`），尺寸/圆形半透明底与删除按钮一致。
- **背景替换流程**:
  - 点击按钮弹出底部弹层，提供“图片背景 / 纯色背景”两类入口。
  - 图片背景：调用系统图片选择器；纯色背景：提供预设色块列表（语义化颜色资源）。
  - 选择后进入“处理中”状态：全屏遮罩 + 进度提示，期间禁用手势与删除。
  - 处理完成展示预览，提供“保存 / 取消”。
  - 保存为新图写入相册，不覆盖原图；失败需提示原因。
...
- **背景替换**:
  - 入口在 `PhotoFragment` 顶部左侧，与删除按钮视觉一致。
  - 处理中必须可见且可取消，完成后提示保存结果。
- **删除流程**:
